...
class LoadFaceRegionsOperator(bpy.types.Operator):
    """Load face regions data for selected face type"""
    bl_idname = "object.load_face_regions"
    bl_label = "Load Face Regions"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        face_type = context.scene.br_face_type
        json_file = f"humanracefacialboneregions{face_type}.json"
        csv_file = "humanracefacialboneregionsmapping.csv"
        
        from ..utils import utils_blender
        json_path = os.path.join(utils_blender.PluginAssetsFolderPath(), json_file)
        csv_path = os.path.join(utils_blender.PluginAssetsFolderPath(), csv_file)
        
        if not os.path.exists(json_path) or not os.path.exists(csv_path):
            self.report({'ERROR'}, f"Face data files not found: {json_file}, {csv_file}")
            return {'CANCELLED'}
        
        try:
            BoneRegionsReader.__bone_regions_data__.import_from_file(json_path, csv_path)
            self.report({'INFO'}, f"Loaded {face_type} face regions data")
        except Exception as e:
            self.report({'ERROR'}, f"Failed to load face data: {str(e)}")
            return {'CANCELLED'}
        
        return {'FINISHED'}

class UnloadFaceRegionsOperator(bpy.types.Operator):
    """Unload face regions data"""
    bl_idname = "object.unload_face_regions"
    bl_label = "Unload Face Regions"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        BoneRegionsReader.__bone_regions_data__.clear()
        self.report({'INFO'}, "Face regions data unloaded")
        return {'FINISHED'}
...
__classes__ = [
    ...
    LoadFaceRegionsOperator,
    UnloadFaceRegionsOperator,
]
